# 06. Preserve Expiration Metadata in the URL Cache
...
`{shortKey}` is the Redis Cluster hash tag, followed by a suffix that identifies
the purpose. The new key string therefore does not overlap the old key. There
is no need to bulk-delete old keys before deployment; they are no longer read
and are eventually removed by TTL or eviction.
